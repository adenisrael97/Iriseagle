...
/**
 * SAFE URL helper (prevents [object Object] bugs)
 */
const safeUrl = (path) => {
...
export function organizationSchema() {
  return {
    ...
    logo: {
      "@type": "ImageObject",
      "@id": `${SITE_URL}/#logo`,
      url: safeUrl(SITE_CONFIG.logo),
      contentUrl: safeUrl(SITE_CONFIG.logo),
      caption: SITE_CONFIG.name,
    },

    image: safeUrl(SITE_CONFIG.ogImage),
...
export function vehicleSchema(vehicle) {
  const schema = {
    ...
    url: safeUrl(vehicle.url),
  };

  // FIXED IMAGE HANDLING
  if (vehicle.image) {
    schema.image = safeUrl(vehicle.image);
  }
...
export function articleSchema(article) {
  return {
    ...
    mainEntityOfPage: safeUrl(article.url),
  };
}
