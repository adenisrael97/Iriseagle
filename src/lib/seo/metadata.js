...
/**
 * SAFE URL HANDLER (fixes [object Object] bugs)
 */
const safeUrl = (path) => {
  if (!path) return "";
  ...
  if (typeof path === "object") {
    return safeUrl(path.url || path.src || "");
  }
...
export function buildOpenGraph({
  ...
  return {
    ...
    images: [
      {
        url: safeUrl(image ?? SITE_CONFIG.ogImage),
        width: 1200,
        height: 630,
        alt: imageAlt ?? title,
      },
...
export function buildTwitterCard({ title, description, image }) {
  return {
    ...
    title,
    description,
    images: [safeUrl(image ?? SITE_CONFIG.ogImage)],
  };
}
...
export function buildVehicleMetadata({
  ...
  const desc =
    ...
    `Buy ${conditionLabel} ${year ?? ""} ${make} ${model} in Nigeria. ${
      price ? `Starting from ${price}. ` : ""
    }Trusted car dealer in Lagos, Abuja & Port Harcourt.`;
...
export function buildRootMetadata() {
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: safeUrl(SITE_CONFIG.ogImage),
          width: 1200,
          height: 630,
    ...
    twitter: {
      ...
      title: `${SITE_CONFIG.name} | ${SITE_CONFIG.tagline}`,
      description: SITE_CONFIG.description,
      images: [safeUrl(SITE_CONFIG.ogImage)],
    },
  };
}