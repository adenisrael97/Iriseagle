...
export default function NewVehicle() {
  ...
  return (
    <section className="bg-white py-10 md:py-14 flex flex-col items-center justify-center px-4">
      <SectionHeading
        title="Stay ahead of the curve"
        subtitle="Introducing the all new 2026 luxury vehicles and buses"
        size="lg"
      ...
      >
        <div className="flex flex-col md:flex-row items-center w-full gap-4 md:gap-0">
          <div className="relative w-full md:w-[60%] h-64 sm:h-80 md:h-128">
            ...
            <Link
              href="/contact"
              className="relative block w-full h-full group focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600 rounded-xl"
            >
              <Image
                src={src}
                alt={name}
          ...
          <div className="flex-1 flex flex-col items-center md:items-start justify-center md:pl-12 pt-2 md:pt-0">
            <p className="font-semibold text-xl sm:text-2xl md:text-3xl text-gray-800 mb-3 w-full text-center md:text-left">
              {name}
            </p>
            <p className="text-gray-600 mb-6 text-sm sm:text-base md:text-lg max-w-sm md:max-w-xl w-full text-center md:text-left">
              {desc}
            </p>
            <Button href="/contact" variant="primary" size="md" shape="pill">
              Request Quote
            </Button>
          </div>
        </div>
        ...
        {vehicles2026.map((_, idx) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-colors ${
              idx === safeIndex ? "bg-red-600" : "bg-gray-300 hover:bg-gray-400"
            }`}
          />
        ))}
      </div>
